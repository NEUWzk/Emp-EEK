...
#使用empathy_mental_health数据集，测试三种得分
...
def train_or_eval_model(model, dataloader, optimizer=None, train=False):
    ...
    for context, response, emp_labels in tqdm(dataloader, leave=False):
        ...
        logits = model(context, response)    #batch,2
    ...
    preds  = np.concatenate(preds)  #emo:2489
    labels = np.concatenate(labels)    #emo:2489
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dim", default="int", help="Which empathetic dimension")
    ...
    # model_save_path = "./models/empathy_1_model.pt"  # 对应emo,valid_acc is 84.71,valid_fscore is 84.68
    # loaded_paras1 = torch.load(model_save_path)
    # model.load_state_dict(loaded_paras1)
    # model.eval()
    #
    #
    ...
    # print(f'valid_loss is {valid_loss},valid_acc is {valid_acc},valid_fscore is {valid_fscore}')
    # print("\n")

    #valid-exp
    # model = T5EncoderClassifier(size).cuda()
    # model_save_path = "./models/empathy_2_model.pt"  # 对应exp,valid_acc is 94.28,valid_fscore is 94.44
    # loaded_paras1 = torch.load(model_save_path)
    # model.load_state_dict(loaded_paras1)
    # model.eval()
    #
    ...
    #valid-int
    model = T5EncoderClassifier(size).cuda()
    model_save_path = "./models/empathy_3_model.pt"  # 对应int,valid_acc is 87.77,valid_fscore is 87.77
    loaded_paras1 = torch.load(model_save_path)
    model.load_state_dict(loaded_paras1)
    model.eval()
