...
from model_2 import EmpEEK
from model_woGCN import wo_GCN
from wo_Decoder import EEK_woDecoder
from wo_Exem import EEK_woExem
...
if __name__ == "__main__":  #这个是目前正确的main函数，用于原始模型已经消融实验、按照epoch保存模型
    ...
    model_save_path = '/root/Emp-EGP/save/wo_exem'
    model = make_model(vocab, 3) #wo_decoder时这里改成1
    model = model.to(config.device)
    isTest = True  #测试时，isTest = True
    if isTest is False:
        model = model.train()
        best_ppl = 1000
        ...
        model = model.to(config.device)
        model = model.eval()
        checkpoint = torch.load("/root/Emp-EGP/save/wo_exem/model_10_40.3438.tar",
