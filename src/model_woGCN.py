...
class wo_GCN(nn.Module):
    ...
    def forward(self, batch):  # 外部知识
        ...
        return mask_src1, act_encoder_outputs
    ...
    def train_one_batch(self, batch, iter, train=True):  #修改后的，自己的
        ...
        mask_src,act_encoder_outputs = self.forward(batch)  #外部知识
        #mask_src : batch,1,src_len
        final_exemp_representations = self.exemplars_forward(batch)  #范例
        final_cat_hidden_states = torch.cat([act_encoder_outputs,  # 不包含concept的上下文编码结果
                                             final_exemp_representations.unsqueeze(1).expand
                                             (-1, act_encoder_outputs.shape[1], -1)], dim=2)
        final_cat_out = self.fc_cat_hidden_states(final_cat_hidden_states)  # batch,len,300 带有范例的
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step + 1):
            ...
            logit = self.generator(
                out, attn_dist, enc_batch, extra_zeros, attn_dist_db=None
            )
            # logit = F.log_softmax(logit,dim=-1) #fix the name later
