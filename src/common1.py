...
from get_metrics import get_dist, calc_bleu
import config
...
def get_input_from_batch(batch):
    ...
    enc_lens = batch["context_lengths"]
    enc_txt = batch["context_txt"]
...
def print_custum(emotion, dial, ref, hyp_b, hyp_g, hyp_t, pred_emotions, comet_res):
    res = ""
...
def evaluate(model, data, ty="valid", max_dec_step=30): #测试过程需要进行greedy生成
    outputs = open("/root/Emp-EGP/result.txt", 'w', encoding='utf-8')
    ...
    for j, batch in pbar:
        ...
        #     print("stop!")
        if config.model == "cem":
            loss, ppl, bce_prog, acc_prog, top_preds, comet_res = model.train_one_batch(  #loss=ctx_loss.item()
                batch, 0, train=False
            )
        else: #记住走这个
            loss, ppl, bce_prog, acc_prog, div_loss = model.train_one_batch(batch, 0, train=False)
        l.append(loss)
        p.append(ppl)
        bce.append(bce_prog)
        acc.append(acc_prog)
        ...
        if ty == "test":
            ...
            # if config.model != "empdg":  #beam搜索方法
            ...
            for i, greedy_sent in enumerate(sent_g):
                ...
                outputs.write("Pred:{} \n".format(greedy_sent))
                outputs.write("Ref:{} \n".format(rf))  #gold  response
                outputs.write("===============================")
        pbar.set_description("loss:{:.4f} ppl:{:.1f}".format(np.mean(l), math.exp(np.mean(l))))


    loss = np.mean(l)  #用平均损失计算PPL
    ppl = np.mean(p)
    bce = np.mean(bce)
    acc = np.mean(acc)
    div1 = np.mean(div)

    print("EVAL\tLoss\tPPL\tAccuracy\tDIV_LOSS\n")
    print("{}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n".format(ty, loss, math.exp(loss), acc,div1))
    if ty == "test":
        _, _, mi_dist1, mi_dist2, avg_len = get_dist(hyp_g)
        avg_bleu, bleu1, bleu2, bleu3, bleu4 = calc_bleu(greedy_sentences)
        print("mi_dist1\tmi_dist2\tavg_len")
        print("{:.4f}\t{:.4f}\t{:.4f}".format(mi_dist1, mi_dist2, avg_len))
        print("avg_bleu\tbleu1\tbleu2\tbleu3\tbleu4")
        print("{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}".format(avg_bleu, bleu1, bleu2, bleu3, bleu4))
