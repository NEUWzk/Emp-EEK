...
class EmpEEK(nn.Module):
    ...
    def forward(self, batch):  # 外部知识
        ...
        return mask_src, act_encoder_outputs, context_add_concept1
    ...
    def train_one_batch(self, batch, iter, train=True):  #修改后的，自己的
        ...
        mask_src,act_encoder_outputs,context_add_concept = self.forward(batch)  #context_add_concept:batch,src_len,300

        final_exemp_representations = self.exemplars_forward(batch)  #范例
        final_cat_hidden_states = torch.cat([context_add_concept,  # (16,src_len,600)
                                             final_exemp_representations.unsqueeze(1).expand
                                             (-1, context_add_concept.shape[1], -1)], dim=2)
        final_cat_out = self.fc_cat_hidden_states(final_cat_hidden_states)  # batch,src_len,300 带有范例的
        ...
        #decode with emo_signal: 训练阶段的解码过程
