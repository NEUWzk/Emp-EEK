...
class ActLatent(nn.Module):
    ...
    def __init__(self,is_eval):
        ...
        self.var = PositionwiseFeedForward(config.hidden_dim, config.filter, config.latent_dim,
                                           ...
                                           dropout=0)
        #后验
        self.mean_p = PositionwiseFeedForward(config.hidden_dim, config.filter, config.latent_dim,
                                                                 layer_config='lll', padding = 'left',
                                                                 dropout=0)
        self.var_p = PositionwiseFeedForward(config.hidden_dim, config.filter, config.latent_dim,
                                                                 layer_config='lll', padding = 'left',
                                                                 dropout=0)
    ...
    def forward(self,x,x_p,act_emb, train=True):
        ...
        #从先验分布中采样隐变量（重参数化方法）：z = 均值+eps x 标准差
        z = eps * std + mean
        kld_loss = 0
        # if x_p is not None:
        #     mean_p = self.mean_p(torch.cat((x_p,act_emb),dim=-1))
        #     log_var_p = self.var_p(torch.cat((x_p,act_emb),dim=-1))
        #     kld_loss = gaussian_kld(mean_p,log_var_p,mean,log_var)#就是KL，没有负号
        #     kld_loss = torch.mean(kld_loss)#一个batch算一个平均的loss

        if x_p is not None:
            mean_p = self.mean_p(x_p)
            log_var_p = self.var_p(x_p)
            kld_loss = gaussian_kld(mean_p,log_var_p,mean,log_var)#就是KL，没有负号
            kld_loss = torch.mean(kld_loss)#一个batch算一个平均的loss
        #如果是训练，则z从后验分布中采样
        if train:
...
def gaussian_kld(recog_mu, recog_logvar, prior_mu, prior_logvar):
    kld = -0.5 * torch.sum(1 + (recog_logvar - prior_logvar)#公式5.23 书上99页
...
def gen_timing_signal(length, channels, min_timescale=1.0, max_timescale=1.0e4):
    ...
    生成由正弦曲线组成的 [1, max_length, hidden_size] 定时信号
    """
...
def get_attn_subsequent_mask(size):
    ...
    subsequent_mask = np.triu(np.ones(attn_shape), k=1).astype('uint8')  # uint8是无符号八位整型，表示范围是[0, 255]的整数
...
class MultiHeadAttention(nn.Module):
    ...
    def _split_heads(self, x):
        """
        Split x such to add an extra num_heads dimension将多头的信息融入x中
        Input:
    ...
    def _merge_heads(self, x):
        """
        Merge the extra num_heads into the last dimension 转换成原来的维度
        Input:
    ...
    def forward(self, queries, keys, values, mask):
        ...
        if mask is not None:
            ...
            logits = logits.masked_fill(mask, -1e18)  # 负无穷大
...
class TransKnowEncoder(nn.Module):
    ...
    def __init__(self,embedding_size, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
        ...
        self.last_layer = EncoderKnowLastLayer(*params)#双多头注意力会调用两次这个层
        ...
        if(config.act):
            ...
            self.n_updates = None
    def forward(self,inputs,inputs_x, mask):#input:[batch,length,emb_dim) mask:[batch,1,length]
        #Add input dropout           input:话语嵌入+说话者嵌入 mask:对pad进行mask：0
        ...
        if(self.universal):
            ...
            # Add timing signal 位置嵌入
            x += self.timing_signal[:, :inputs.shape[1], :].type_as(inputs.data)#在torch.Size([1, 1000, 300])上进行切片
            inputs_x += self.timing_signal[:, :inputs_x.shape[1], :].type_as(inputs_x.data)#在torch.Size([1, 1000, 300])上进行切片
            # inputs_x += self.timing_signal[:, :inputs_x.shape[1], :].type_as(inputs_x.data)#在torch.Size([1, 1000, 300])上进行切片
    ...
    def forward(self, inputs, inputs_x, mask_src, mask_x):
        ...
        if (self.universal):
            ...
            inputs_x += self.timing_signal[:, :inputs_x.shape[1], :].type_as(
                inputs_x.data)  # 在torch.Size([1, 1000, 300])上进行切片
            # inputs_x += self.timing_signal[:, :inputs_x.shape[1], :].type_as(inputs_x.data)#在torch.Size([1, 1000, 300])上进行切片
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, encoder_output, k_scores, mask, node_feature, node_mask):
        ...
        # Run decoder attn_dist为enc_dec的注意力权重
        ...
        return y, ky, attn_dist, node_att_dist
...
def get_input_from_batch(batch):
    ...
    # 把该批次的句子都填充到最大的句子长度，mask为：是真实的话语单词就是1 是padding的就是0
