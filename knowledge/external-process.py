...
def gen_embeddings(n_words, word2index, emb_dim=300, emb_file='glove.6B.300d.txt'):
    ...
    """
    embeddings = np.random.randn(n_words, emb_dim) * 0.01  #0.01?
    ...
    if emb_file is not None:
        ...
        for line in open(emb_file).readlines():
            sp = line.split()
            if(len(sp) == emb_dim + 1):
                if sp[0] in word2index:  #sp[0]表示这个单词 后面300维是具体的向量 比如单词'the'
                    pre_trained += 1
                    # a = sp[0]  #'the'
                    # b = word2index[sp[0]]  #word2index['the']  ->13
                    # c = embeddings[b]
...
def get_concept_dict():
    ...
    embeddings = gen_embeddings(n_words, word2index) #(vocab_len,300) 词嵌入，tensor值特别小
    ...
    for i, row in enumerate(CN):
        ...
        if c1_lang == "en" and c2_lang == "en": #语言类型
            ...
            if c1 in word2index and c2 in word2index and c1 != c2 and c1.isalpha() and c2.isalpha():
                ...
                v1, a1, d1 = VAD[c1] if c1 in VAD else [0.5, 0.0, 0.5] #获取VAD向量
    ...
    print("concept num: ", len(concept_dict)) #14995
...
def rank_concept_dict():
    ...
    rank_concept_file = open('ConceptNet_VAD_dict.json', 'w', encoding='utf-8')  #排名后的ConceptNet字典
...
def read_our_dataset(concept_num=3, total_concept_num=10,):
    with open('EmpatheticDialogue/dataset_preproc.json', "r") as f:
        [data_tra, data_val, data_tst, vocab] = json.load(f) #此时还没有额外知识
        word2index, word2count, index2word, n_words = vocab

    VAD = json.load(open("VAD.json", "r", encoding="utf-8")) #也是一个字典：“word”:[V,A,D]
    ...
    for i, sample in enumerate(train_contexts):  #sample:[[]] 二维list
        ...
        for j, sentence in enumerate(sample):  #sentence:每一个单轮对话 即一维[]
            ...
            for cti, uc in enumerate(sentence_concepts):  #sentence_concepts和输入的对话长度一致 # filter concepts of each token, complete their VAD value, select top total_concept_num.
...
if __name__ == '__main__':
    ...
    with open('./kemp_dataset_preproc_new.json', "r") as f:
    #     # pickle.dump([data_tra, data_val, data_tst, vocab], f)
    #     # print("Saved kemp_dataset_preproc.p file.")
    #     pickle.dump([data_tst,vocab],f)
        data = json.load(f)
    print("--")
